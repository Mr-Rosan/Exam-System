    ...
    <div class="layuimini-main">
        <c:if test="${modify eq 'false'}">
            <blockquote class="layui-elem-quote layui-text">
                创建考试等级，创建考试等级之后生成的等级编号告诉学生。学生通过老师给的考试码申请考试等级
            </blockquote>
        </c:if>
        <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
            <c:choose>
                <c:when test="${modify.equals('true')}">
                    <legend>修改考试等级</legend>
                </c:when>
                <c:otherwise>
                    <legend>创建考试等级</legend>
                </c:otherwise>
            </c:choose>
        </fieldset>
        ...
        <form class="layui-form" method="post" action="" lay-filter="example">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">考试等级名称</label>
            ...
            <div class="layui-form-item layui-form-text">
                <label class="layui-form-label">考试等级介绍</label>
                <div class="layui-input-block">
            ...
            <div class="layui-form-item">
                <div class="layui-input-block">
                    <c:choose>
                        <c:when test="${modify.equals('true')}">
                            <button class="layui-btn" lay-submit="" lay-filter="saveBtn">修改考试等级</button>
                        </c:when>
                        <c:otherwise>
                            <button class="layui-btn" lay-submit="" lay-filter="saveBtn">创建考试等级</button>
                        </c:otherwise>
                    </c:choose>
